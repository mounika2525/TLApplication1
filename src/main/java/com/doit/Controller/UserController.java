...
//    private SecurityService securityService;
    ...
    public  List<User> byUId(@PathVariable(value = "userId") Long userId){
    ...
    @RequestMapping(value = "/usr/{username}",method = RequestMethod.POST)
    ...
    @RequestMapping(value="/usrd" ,method = RequestMethod.POST)
    public User getByEmailAndPassword(@RequestParam String email,@RequestParam String password){
        User user=useService.findByEmailAndPassword(email,password);
        if(user.getEmail().equals(email) && user.getPassword().equals(password))
            return  user;
        else throw new HandleException("no such user",new RuntimeException());
    }
//    @RequestMapping(value="/usrd" ,method = RequestMethod.POST)
//    public User getByEmailAndPassword(@RequestParam String email,@RequestParam String password){
//        User user=useService.findByEmailAndPassword(email,password);
...
//        else throw new HandleException("no such user",new RuntimeException());
//    }
//    public User getByUserAndPass(@RequestBody User user){
//        User user1=useService.find
//    }
